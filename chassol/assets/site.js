  ...
  const update=()=>{
    ...
    if(brand && hasHero){
      ...
      const startWidth=mobile
        ? Math.min(window.innerWidth*.54,230)
        : Math.min(window.innerWidth*.26,390);
      ...
      /* Finish the morph before leaving the hero */
      const distance=Math.max(260,window.innerHeight*.46);
      const t=clamp(y/distance,0,1);
      const eased=1-Math.pow(1-t,3);

      /* important is intentional because older CSS versions used !important */
